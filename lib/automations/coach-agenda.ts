...
function agendaEmail(input: { localDate: string; timezone: string; sessions: AgendaSession[]; pendingChanges: number; pendingCheckins: number }) {
  ...
  const sessionRows = input.sessions.length ? input.sessions.map((session) => {
    const formatter = new Intl.DateTimeFormat("es-ES", { timeZone: input.timezone, hour: "2-digit", minute: "2-digit" });
  ...
  return {
    subject,
    html: `<!doctype html><html lang="es"><body style="margin:0;background:#eef2f8;font-family:Arial,sans-serif"><table role="presentation" width="100%"><tr><td align="center" style="padding:38px 14px"><table role="presentation" width="100%" style="max-width:640px;background:#050914;border:1px solid #1b2940;border-radius:22px;overflow:hidden"><tr><td style="height:5px;background:linear-gradient(90deg,#2f6bff,#00d4ff)"></td></tr><tr><td style="padding:36px 40px"><img src="${RG_LOGO_URL}" width="250" alt="RG Coach" style="max-width:100%;height:auto"><p style="display:inline-block;margin:28px 0 12px;padding:7px 10px;border:1px solid #7a342f;border-radius:999px;background:#2b1212;color:#ff9b91;font-size:10px;font-weight:900;letter-spacing:1.4px">IMPORTANTE · RESUMEN AUTOMÁTICO 22:00</p><h1 style="margin:0;color:#fff;font-size:34px">HORARIOS DE MAÑANA</h1><p style="margin:9px 0 0;color:#9cabc1;font-size:15px;text-transform:capitalize">${escapeHtml(dateLabel)} · HORA DE MIAMI</p><table role="presentation" width="100%" style="margin-top:22px;border-collapse:collapse"><tr><th align="left" style="padding:9px 0;border-bottom:2px solid #2f6bff;color:#718099;font-size:9px;letter-spacing:1px">HORARIO</th><th align="left" style="padding:9px 12px;border-bottom:2px solid #2f6bff;color:#718099;font-size:9px;letter-spacing:1px">CLIENTE</th><th align="right" style="padding:9px 0;border-bottom:2px solid #2f6bff;color:#718099;font-size:9px;letter-spacing:1px">UBICACIÓN</th></tr>${sessionRows}</table>${input.pendingChanges ? `<p style="margin:24px 0 0;padding:16px;border:1px solid #7d5d22;border-radius:12px;background:#281f0e;color:#fff"><strong style="color:#f4c46a;font-size:15px">⚠ ${input.pendingChanges} CAMBIO${input.pendingChanges === 1 ? "" : "S"} PENDIENTE${input.pendingChanges === 1 ? "" : "S"}</strong><br><span style="font-size:13px;color:#c8b991">Revisa y aprueba o rechaza la solicitud antes de cerrar el día.</span></p>` : `<p style="margin:24px 0 0;padding:14px;border:1px solid #205a3d;border-radius:12px;background:#10261d;color:#68e0a0;font-size:13px;font-weight:800">✓ SIN CAMBIOS DE HORARIO PENDIENTES</p>`}${input.pendingCheckins ? `<p style="margin:14px 0 0;padding:16px;border:1px solid #2a4a7d;border-radius:12px;background:#0d1a2e;color:#fff"><strong style="color:#7db2ff;font-size:15px">📋 ${input.pendingCheckins} CHECK-IN${input.pendingCheckins === 1 ? "" : "S"} SIN REVISAR</strong><br><span style="font-size:13px;color:#93a9c8">Tus clientes esperan feedback. Revísalos en la consola.</span></p>` : ""}<a href="${dashboardUrl}" style="display:inline-block;margin-top:26px;padding:15px 21px;border-radius:11px;background:#2f6bff;color:#fff;text-decoration:none;font-weight:900">ABRIR CONSOLA Y REVISAR →</a><p style="margin:30px 0 0;color:#64748b;font-size:11px;line-height:1.5">Envío automático de RG Coach a rubengomezesp@gmail.com. Se genera una sola vez cada día a las 22:00, hora de Miami.</p></td></tr></table></td></tr></table></body></html>`,
    text: [`Agenda RG · ${dateLabel}`, "", ...(textLines.length ? textLines : ["Sin entrenamientos confirmados."]), "", `${input.pendingChanges} solicitudes de cambio pendientes.`, `${input.pendingCheckins} check-ins sin revisar.`, dashboardUrl].join("\n"),
  };
}

async function sendDigest(input: { to: string; localDate: string; timezone: string; sessions: AgendaSession[]; pendingChanges: number; pendingCheckins: number }) {
  ...
  try {
    const response = await fetch(RESEND_EMAILS_URL, {
      ...
      body: JSON.stringify({ from, to: [input.to], subject: message.subject, html: message.html, text: message.text, headers: { "X-Priority": "1", "Importance": "high", "X-MSMail-Priority": "High" }, tags: [{ name: "workspace", value: "rg-coach" }, { name: "message_type", value: "coach_agenda_digest" }] }),
...
export async function runCoachAgendaAutomations(now = new Date()): Promise<CoachAgendaAutomationResult> {
  ...
  for (const config of configs) {
    ...
    const [sessionsResult, changesResult, checkinsResult] = await Promise.all([
      ...
      // Red de seguridad del email inmediato: los check-ins sin revisar reaparecen aquí.
      db.from("customer_checkins").select("id", { count: "exact", head: true }).eq("workspace_id", config.workspace_id).neq("status", "reviewed"),
    ]);
    ...
    const pendingCheckins = checkinsResult.error ? 0 : (checkinsResult.count ?? 0);
    const delivery = await sendDigest({ to: config.recipient_email, localDate: tomorrow, timezone: config.timezone, sessions, pendingChanges, pendingCheckins });
