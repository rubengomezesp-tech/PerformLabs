 ...
 * (hero → barriers → what's included → benefits → about → plans) that reskins per coach
 * via `--accent`/`--sales-bg` and the brand's hero/logo/about copy. Presentational only —
 * the route resolves brand/plans and passes the server checkout action (untouched).
 */
  ...
  return (
    <main className="salesPage" style={{ "--accent": accent, "--sales-bg": background } as CSSProperties}>
      ...
      <section id="planes" className="salesSection salesPlans" aria-labelledby="sales-plans-title">
        ...
        {notice ? (
          <p className="formMessage danger" role="alert">{notice}</p>
        ) : null}
