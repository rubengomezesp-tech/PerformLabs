...
const PRIMARY_MENU = [
  ...
  { label: "Mensajes", href: "/app/support", icon: MessageSquare },
];
...
const ACCOUNT_MENU = [
  ...
  { label: "Mi ficha", href: "/app/onboarding", icon: ClipboardList },
];

const MENU_SECTIONS = [
  { label: "Principal", items: PRIMARY_MENU },
  { label: "Más herramientas", items: TOOL_MENU },
  { label: "Cuenta", items: ACCOUNT_MENU },
];

const MENU = MENU_SECTIONS.flatMap((section) => section.items);
        ...
        <div className="memberDrawer" role="dialog" aria-modal="true" aria-label={brand?.name ?? "Menú"}>
          ...
          <nav className="memberDrawerPanel" ref={panelRef} tabIndex={-1}>
            ...
            {MENU_SECTIONS.map((section) => (
              <div className="memberDrawerGroup" key={section.label}>
                <span className="memberDrawerGroupLabel">{section.label}</span>
